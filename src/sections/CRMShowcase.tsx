...
export function CRMShowcase() {
  return (
    <SectionTransition className="py-24 md:py-32 bg-primary relative z-10 overflow-hidden">
      {/* Background glow */}
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          <div>
            ...
            <p className="text-lg text-sub mb-10 leading-relaxed">
            ...
            <div className="glass-panel p-6 rounded-2xl">
              <p className="text-sm text-sub uppercase tracking-wider mb-4 font-medium">Подходит для:</p>
              <div className="flex flex-wrap gap-2">
                {siteData.crmShowcase.industries.map((industry) => (
                  <span key={industry} className="px-3 py-1 rounded-full bg-fg-subtle border border-border-medium text-sm text-muted">
                    {industry}
                  </span>
                ))}
              </div>
          ...
          <div className="relative">
            ...
            <motion.div 
              ...
              className="relative rounded-2xl bg-[#0B1120] border border-border-medium shadow-[0_0_50px_rgba(0,163,255,0.15)] overflow-hidden"
            >
              {/* Fake Chrome */}
              <div className="h-10 bg-fg-subtle border-b border-border-medium flex items-center px-4 gap-2">
              ...
              <div className="p-6 md:p-8 grid gap-6">
                ...
                <div className="flex justify-between items-center mb-4">
                  <div className="h-6 w-32 bg-fg-medium rounded-md" />
                  <div className="h-8 w-8 rounded-full bg-blue-500/20 grid place-items-center text-blue-400 text-xs font-bold">DS</div>
                </div>

                <div className="grid grid-cols-3 gap-4">
                  {[1,2,3].map(i => (
                    <div key={i} className="h-24 bg-fg-subtle rounded-xl border border-border-subtle p-4 flex flex-col justify-between">
                      <div className="h-4 w-1/2 bg-fg-medium rounded" />
                      <div className="h-6 w-3/4 bg-blue-400/20 rounded" />
                    </div>
                  ))}
                </div>

                <div className="h-48 rounded-xl border border-border-medium bg-gradient-to-b from-white/5 to-transparent relative overflow-hidden flex items-end px-4 gap-4 pb-4">
                   {/* Fake chart bars */}
